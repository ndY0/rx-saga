...
the mergeLink operator allow you to chain a command to an observable of commands, while specifying the error propagation.
to summaryze, you can generate multiple commands branching from one. Every Downstream commands is then merged downstream.

```typescript
import SagaSubject from "rx-saga";
import { mergeLink } from "rx-saga/operators";
 ...
 */
const linkedSaga = saga.pipe(mergeLink(map, liftError, mapError, mapResponse, mapErrorResponse))
...
<img src="../assets/mergelinkoperator.drawio.svg">