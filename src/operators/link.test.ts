...
describe('link', () => {
    ...
    it(`should allow to link the error response generated by this new error response to the previous error command`, (done) => {
        ...
        downstream.toObservable().subscribe({
            next(value) {
                count += 1;
                expect(execMock).toHaveBeenCalledTimes(2 * count + 4 * countError);
                expect(execMock).toHaveBeenNthCalledWith(2 * (count - 1) + 1 + 4 * countError, command);
                expect(execMock).toHaveBeenNthCalledWith(2 * (count) + 4 * countError, command2);
                expect(mapSpy).toBeCalledTimes(count + countError);
                expect(mapSpy).toHaveBeenNthCalledWith(count + countError, response);
                expect(value.type).toBeDefined();
                expect(value.type).toEqual('response2');
            },
            complete() {
        ...
        saga.toErrorObservable().subscribe({
            next(value) {
                countError += 1;
                expect(execMock).toHaveBeenCalledTimes(2 * count + 4 * countError);
                expect(execMock).toHaveBeenNthCalledWith(2 * count + 4 * countError, errorCommand);
                expect(execMock).toHaveBeenNthCalledWith(2 * count + 4 * (countError - 1) + 3, errorCommand2);
                expect(execMock).toHaveBeenNthCalledWith(2 * count + 4 * (countError - 1) + 2, command2);
                expect(execMock).toHaveBeenNthCalledWith(2 * count + 4 * (countError - 1) + 1, command);
                expect(liftErrorSpy).toHaveBeenCalledTimes(countError);
                expect(liftErrorSpy).toHaveBeenNthCalledWith(countError, errorResponse2);
                expect(value.type).toBeDefined();
    ...
    it('should propagate unexpected errors upstream', (done) => {
        const mapErrorUpstreamSpy = jest.fn<{ type: 'errorCommand' }, [ErrorMock]>((error: ErrorMock) => ({ type: 'errorCommand' }));
        const mapErrorResponseUpstreamSpy = jest.fn<{ type: 'errorResponse' }, [any]>((errorResponse: any) => ({ type: 'errorResponse' }));
        const mapResponseUpstreamSpy = jest.fn<{ type: 'response' }, [any]>((response: any) => (response));
        const saga = new SagaSubject<{ type: 'command' }, { type: 'response' }, ErrorMock, { type: 'errorCommand' }, { type: 'errorResponse' }>(busMock(), mapErrorUpstreamSpy, mapResponseUpstreamSpy, mapErrorResponseUpstreamSpy);
        ...
        downstream.unexpectedError(error);
        saga.complete();
    });
});